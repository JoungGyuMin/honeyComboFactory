...
│           ├── common/
│           │   ├── config/                   # 스프링 설정 클래스 (DataSource, View 등)
│           │   ├── http/                     # HTTP GET 유틸
│           │   ├── service/                  # 공통 서비스 로직 (결제/장바구니 등)
│           │   ├── transaction/              # 트랜잭션 처리 AOP
│           │   └── util/                     # 암호화, QR, JDBC 유틸 모음

│           ├── external.sms/                 # 문자 인증 서비스 연동

│           ├── view/                         # 사용자 요청 처리 (Controller 계층)
│           │   ├── boardCombo/               # 꿀조합 게시판 처리
│           │   ├── boardComboLiked/          # 좋아요 처리
│           │   ├── member/                   # 회원가입, 로그인, 소셜 로그인 등
│           │   ├── page/                     # 메인, 관리자, 마이페이지 이동 처리
│           │   ├── product/                  # 상품 관련 요청 처리
...
│   ├── static/assets/                   # 정적 리소스 (js, css, img 등)
